...
swmm_inp = "C:/PycharmProjects/swmm_rl_hague/hague_inp_files_pyswmm/hague_v22_passive_gw.inp"
...
with Simulation(swmm_inp) as sim:  # run sim for rpt w/o time series
    ...
    for step in sim:  # loop through all steps in the simulation
        ...
        St1_gwq = swmm_utils.calc_dupuit(St1.depth, St1.invert_elevation, current_gwl, St1_rad)
        # St2_gwq = swmm_utils.calc_gwl(St2.depth - abs(St2.invert_elevation), current_gwl, St2_rad)
        St3_gwq = swmm_utils.calc_dupuit(St3.depth, St3.invert_elevation, current_gwl, St3_rad)
        # print(St1_gwq)
        ...
        # if step_count % control_time_step == 0:  # use this for routing step, not 15min step
...
rpt_df.to_csv("C:/PycharmProjects/swmm_hague_rbc/cmac_results/092016_tssrbc_gw_v22.csv", index=True)
